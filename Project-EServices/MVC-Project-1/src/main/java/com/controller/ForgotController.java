...
public class ForgotController extends HttpServlet {
    ...
    private static final String SENDER_ID = "ansariaman0209@gmail.com";
    private static final String PASSWORD = "jgvcptakwulmtjkk";
    ...
    private void handleOtpSubmission(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        CustomerModel customer = dao.checkEmail(email);

        if (customer == null) {
            request.setAttribute("invalidemail", "Email Address Not Valid");
            request.getRequestDispatcher("cust-forgot.jsp").forward(request, response);
        } else {
            sendOtpEmail(request, response, customer);
        }
    }

    private void sendOtpEmail(HttpServletRequest request, HttpServletResponse response, CustomerModel customer) throws ServletException, IOException {
        try {
    ...
    private void handlePasswordReset(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        HttpSession session = request.getSession(false);
        CustomerModel customer = (CustomerModel) session.getAttribute("UserData");
